  ...
  *Requested by:* ${item.requestor.toUpperCase()}
  ...
  const markUnavailable = async (item) => {
    ...
    try {
      ...
      // --- AUTOMATIC WHATSAPP LOGIC ---
      // Look up the requestor's phone number from your PHONE_MAP
      const requestorPhone = PHONE_MAP[item.Requestor] || "910000000000";
      
      const message = 
  `*ITEM UNAVAILABLE*
  --------------------------------
  *Item:* ${item.ItemName.toUpperCase()} ${item.Spec}
  *Company:* ${item.Company}
  *Status:* Out of Stock / Not Available
  *Owner:* ${item.Owner.toUpperCase()}
  
  *Note:* This request has been moved to the Ordered history as Unavailable.`;
  
      // Open WhatsApp automatically
      window.open(`https://wa.me/${requestorPhone}?text=${encodeURIComponent(message)}`, '_blank');
  ...
  const handleBulkPrint = (allData, type, activeFilters) => {
    try {
      ...
      const filtered = allData.filter(i => {
        const matchStatus = type === 'toOrder' ? i.Status === 'Pending' : (i.Status === 'Completed' || i.Status === 'Unavailable');
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <main className="p-4 max-w-2xl mx-auto">
        ...
        {view === 'add' && <AddForm onSave={() => setView('toOrder')} currentUserEmail={user.email} items={items} />}
...
function AddForm({ onSave, currentUserEmail, items }) {
  const [btnLoading, setBtnLoading] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [lastOrder, setLastOrder] = useState(null);
  const [countdown, setCountdown] = useState(3);
  const [form, setForm] = useState({ itemName: '', company: '', spec: '', qty: '', unit: 'pieces', shop: 'Seva', owner: 'Hussain' });

  // Handle the automatic reset timer
  useEffect(() => {
    let timer;
  ...
  // --- AUTOFILL LOGIC ---
  // Create a unique list of item names from history
  const suggestions = [...new Set(items.map(i => i.ItemName))].filter(Boolean);

  const handleItemNameChange = (e) => {
    const val = e.target.value;
    setForm({ ...form, itemName: val });

    // If the typed name matches a known item exactly, autofill the rest
    const existingItem = items.find(i => i.ItemName?.toLowerCase() === val.toLowerCase());
    if (existingItem) {
      setForm(prev => ({
        ...prev,
        itemName: existingItem.ItemName,
        company: existingItem.Company || prev.company,
        spec: existingItem.Spec || prev.spec,
        unit: existingItem.Unit || prev.unit
      }));
    }
  };
  ...
  const sendWhatsApp = () => {
    ...
    const message = `# *NEW REQUIREMENT*%0A--------------------------------%0A*Item:* ${itemName.toUpperCase()} ${spec ? `(${spec})` : ''}%0A*Qty:* ${qty} ${unit}%0A*Company:* ${company || '-'}%0A*Shop:* ${shop}%0A%0A*Assigned to:* ${owner.toUpperCase()}%0A*Requested by:* ${requestor.toUpperCase()}%0A--------------------------------%0A*Please check the Seva Orders app to mark as completed.*`;
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-green-100 text-center max-w-md mx-auto animate-in fade-in zoom-in duration-300">
        <div className="flex justify-center mb-4">
          <div className="bg-green-100 p-4 rounded-full relative">
            ...
            <div className="absolute -top-2 -right-2 bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-xs border-4 border-white">
              {countdown}
            </div>
          </div>
        </div>
        <h2 className="text-2xl font-black text-gray-800 mb-1 uppercase tracking-tighter">Requirement Added!</h2>
        <p className="text-gray-400 text-xs font-bold mb-8 uppercase tracking-widest">Returning to form in {countdown}s...</p>
        <div className="space-y-3">
          <button onClick={sendWhatsApp} className="w-full bg-[#25D366] text-white py-5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-[0_8px_20px_rgba(37,211,102,0.3)] active:scale-95 transition-all">
          ...
          <button onClick={handleReset} className="w-full bg-gray-100 text-gray-400 py-4 rounded-2xl font-black text-xs uppercase tracking-widest">
            Add Another Now
          </button>
        </div>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-gray-100 space-y-4 max-w-md mx-auto">
      ...
      <div>
        ...
        {/* Added list attribute for datalist */}
        <input 
          required 
          ...
          placeholder="e.g. Aquagel Sunscreen" 
        />
        {/* The Datalist UI */}
      ...
      <button disabled={btnLoading} type="submit" className={`w-full py-5 rounded-2xl font-black text-lg shadow-xl transition-all flex items-center justify-center gap-2 mt-4 ${btnLoading ? 'bg-gray-200 text-gray-400' : 'bg-blue-600 text-white hover:bg-blue-700 active:scale-95'}`}>
...
function ListView({ items, type, onComplete, onUnavailable, onBulkPrint, onDelete, currentUserEmail }) {
  ...
  // UPDATED FILTER: Now includes the Search Term logic
  const filtered = items
    .filter(i => {
      ...
      // SEARCH LOGIC: Checks Item Name, Company, and Specification
      const s = searchTerm.toLowerCase();
      const matchSearch = i.ItemName?.toLowerCase().includes(s) || 
                          i.Company?.toLowerCase().includes(s) || 
                          i.Spec?.toLowerCase().includes(s);

      return matchStatus && matchShop && matchOwner && matchReq && matchSearch;
    })
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-md border border-blue-50 space-y-3">
        ...
        <div className="relative">
          <input 
            type="text"
            placeholder="Search items, companies or specs..."
            className="w-full bg-gray-50 border border-gray-100 p-3 pl-10 rounded-xl text-sm outline-none focus:border-blue-300 transition-all font-medium"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <svg className="absolute left-3 top-3.5 text-gray-400" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="grid grid-cols-3 gap-2">
          <select className="bg-gray-50 p-2 rounded-xl text-[10px] font-black uppercase outline-none border border-transparent focus:border-blue-100" value={filterShop} onChange={(e) => setFilterShop(e.target.value)}>
            <option value="All">All Shops</option>
            <option value="Seva">Seva</option>
            <option value="Seva Mart">Seva Mart</option>
            <option value="Seva Super Store">Seva Super Store</option>
          </select>
          <select className="bg-gray-50 p-2 rounded-xl text-[10px] font-black uppercase outline-none border border-transparent focus:border-blue-100" value={filterOwner} onChange={(e) => setFilterOwner(e.target.value)}>
            <option value="All">All Owners</option>
            {['Hussain', 'Burhan', 'Ali', 'Mohammed', 'Shabbar', 'Huzefa', 'Taha', 'Kamlesh', 'Rahul'].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          <select className="bg-gray-50 p-2 rounded-xl text-[10px] font-black uppercase outline-none border border-transparent focus:border-blue-100" value={filterReq} onChange={(e) => setFilterReq(e.target.value)}>
        ...
        <button onClick={() => onBulkPrint(items, type, { shop: filterShop, owner: filterOwner, requestor: filterReq })} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all">
          <Printer size={16} className="inline mr-2 mb-0.5"/> PRINT FILTERED LIST (A5)
        </button>
      </div>

      {Object.keys(grouped).length === 0 ? (
        <div className="text-center py-10 text-gray-400 font-bold uppercase text-[10px] tracking-[0.2em]">No matches found</div>
      ) : (
          ...
          <section key={month} className="mb-6">
            ...
            <div className="space-y-3">
              {grouped[month].map((item, idx) => {
                ...
                return (
                  <div key={idx} className="relative overflow-hidden rounded-2xl mb-3">
                    {item.Status === 'Unavailable' && (
                      <div className="absolute inset-0 bg-white/60 backdrop-blur-[0.2px] z-10 flex items-center justify-center pointer-events-none">
                        <div className="flex flex-col items-center gap-1">
                          <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] bg-orange-50 px-4 py-1.5 rounded-full border border-orange-100 shadow-sm">
                            Not Available
                          </span>
                        </div>
                      </div>
                    ...
                    <div className={`bg-white p-5 border border-gray-100 flex justify-between items-center transition-all ${item.Status === 'Unavailable' ? 'opacity-80' : ''}`}>
                      <div className="flex-1 space-y-3 pr-4">
                        ...
                        <div className="flex flex-wrap gap-2 pt-2.5 border-t border-gray-100">
                          {item.Status === 'Unavailable' ? (
                            <span className="text-[8px] font-black bg-orange-600 text-white px-2 py-1 rounded uppercase tracking-wider">Unavailable</span>
                          ) : (
                            type === 'ordered' && (
                              <span className="text-[8px] font-black bg-green-50 text-green-600 px-2 py-1 rounded border border-green-100 uppercase tracking-wider">
                                {getHours(item.Date, item.CompletedAt)}
                              </span>
                            )
                          )}
                          <span className="text-[8px] font-black bg-gray-100 text-gray-600 px-2 py-1 rounded border border-gray-200 uppercase">{new Date(item.Date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                          ...
                          <span className="text-[8px] font-black bg-indigo-50 text-indigo-600 px-2 py-1 rounded border border-indigo-100 uppercase">Req: {item.Requestor}</span>
                        </div>
                      </div>
                      ...
                      {type === 'toOrder' && (
                        <div className="flex flex-col gap-2 relative z-20">
                          ...
                          <button disabled={!isAssignedOwner} onClick={() => onComplete(item)} className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-all ${isAssignedOwner ? "bg-green-50 text-green-600 border-green-100 active:scale-95" : "bg-gray-50 text-gray-300 border-gray-100 opacity-50 cursor-not-allowed"}`}><Check size={20} strokeWidth={3}/></button>
                          {isAssignedOwner && (
                            <button onClick={() => onUnavailable(item)} className="w-10 h-10 flex items-center justify-center rounded-xl border transition-all bg-orange-50 text-orange-600 border-orange-100 active:scale-95 shadow-sm"><span className="font-bold text-xs uppercase text-center leading-none">N/A</span></button>
                          )}
                          <button disabled={!isRequestor} onClick={() => onDelete(item)} className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-all ${isRequestor ? "bg-red-50 text-red-500 border-red-100 active:scale-95" : "bg-gray-50 text-gray-300 border-gray-100 opacity-50 cursor-not-allowed"}`}><span className="font-bold text-xl leading-none">×</span></button>
                        </div>
                      )}
                    </div>
                  </div>
