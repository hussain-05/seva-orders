              ...
              {grouped[month].map((item, idx) => {
                ...
                return (
                  <div key={idx} className="relative overflow-hidden rounded-2xl mb-3">
                    {/* MINIMALIST OVERLAY FOR UNAVAILABLE */}
                    ...
                    <div className={`bg-white p-5 border border-gray-100 flex justify-between items-center transition-all ${item.Status === 'Unavailable' ? 'opacity-80' : ''}`}>
                      <div className="flex-1 space-y-3 pr-4">
                        ...
                        <div className="flex flex-wrap gap-2 pt-2.5 border-t border-gray-100">
                          {item.Status === 'Unavailable' ? (
                            <span className="text-[8px] font-black bg-orange-600 text-white px-2 py-1 rounded uppercase">Unavailable</span>
                          ) : (
                            type === 'ordered' && (
                      ...
                      {/* BUTTON COLUMN - UPDATED VISIBILITY LOGIC */}
                      {type === 'toOrder' && (
                        <div className="flex flex-col gap-2 relative z-20">
                          
                          {/* 1. WHATSAPP REMINDER: Hidden if not the requestor */}
                          {isRequestor && (
                            <button 
                              onClick={() => handleCardWhatsApp(item)} 
                              className="w-10 h-10 flex items-center justify-center rounded-xl border bg-green-50 text-[#25D366] border-green-100 shadow-sm active:scale-90 transition-all"
                            >
                          ...
                          {/* 2. COMPLETE BUTTON: Always visible, but disabled if not the owner */}
                          <button 
                          ...
                          {/* 3. N/A BUTTON: Hidden if not the owner */}
                          {isAssignedOwner && (
                            <button 
                              onClick={() => onUnavailable(item)} 
                              className="w-10 h-10 flex items-center justify-center rounded-xl border transition-all bg-orange-50 text-orange-600 border-orange-100 active:scale-95 shadow-sm"
                            >
                              <span className="font-bold text-xs uppercase text-center leading-none">N/A</span>
                            </button>
                          )}

                          {/* 4. DELETE BUTTON: Always visible, but disabled if not the requestor */}
                          <button 
                            disabled={!isRequestor}
                            onClick={() => onDelete(item)} 
                            className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-all ${
                              isRequestor 
                                ? "bg-red-50 text-red-500 border-red-100 active:scale-95" 
                                : "bg-gray-50 text-gray-300 border-gray-100 opacity-50 cursor-not-allowed"
                            }`}
                          >
